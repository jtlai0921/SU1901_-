...
public class HighlighterTest {
	public static void main(String[] args) throws IOException, InvalidTokenOffsetsException, ParseException {
		...
		SimpleHTMLFormatter fors = new SimpleHTMLFormatter("<span style=\"color:red;\">", "</span>");// 定制反白標簽
		Highlighter highlighter = new Highlighter(fors, score);// 反白分析器
		...
		for (ScoreDoc sd : tds.scoreDocs) {
			...
			TokenStream tokenStream = TokenSources.getAnyTokenStream(searcher.getIndexReader(), sd.doc, field,
					analyzer);// 取得tokenstream
			Fragmenter fragment = new SimpleSpanFragmenter(score);
			highlighter.setTextFragmenter(fragment);
			String str = highlighter.getBestFragment(tokenStream, doc.get(field));// 取得反白的部分
			System.out.println("反白的部分:" + str);
		}
		dir.close();
		reader.close();
	}
